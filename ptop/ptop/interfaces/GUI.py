...
class Ptop(npyscreen.Form):
    ...
    def update(self):
        ...
        try:
            ...
            row4 = "Network Speed  {2}{0: <3}↓ {1: <3}↑ MB/s".format(
            ...
            # check sorting flags
            if MEMORY_SORT:
                sorted_processes_data = sorted(
                    self._processes_data, key=lambda k: k["memory"], reverse=True
                )
                self._logger.info("Memory sorting done for process table")
            elif TIME_SORT:
                sorted_processes_data = sorted(
                    self._processes_data, key=lambda k: k["rawtime"], reverse=True
                )
                self._logger.info("Time sorting done for process table")
            elif PROCESS_RELEVANCE_SORT:
                sorted_processes_data = sorted(
                    self._processes_data, key=lambda k: k["rawtime"]
                )
            ...
            for proc in sorted_processes_data:
                curtailed_processes_data.append(
                    "{0: <30} {1: >5}{6}{2: <10}{6}{3}{6}{4: >6.2f} % {6}{5}\
                ".format(
                        (
                            (proc["name"][:25] + "...")
                            if len(proc["name"]) > 25
        ...
        # catch the friggin KeyError caused to c
