...
#SBATCH --job-name=mlperf-inference-text-to-image-stable-diffusion-xl-python-datacenter-pytorch-cpu
...
declare -xr CONDA_ENV_YAML="${CONDA_CACHE_DIR}/mlperf-inference-text-to-image-stable-diffusion-xl-python-datacenter-pytorch-cpu.yaml"
...
if [[ -f "${CONDA_ENV_YAML}.md5" ]] && md5sum -c "${CONDA_ENV_YAML}.md5"; then 
  ...
  conda activate base
  conda install -y mamba -n base --override-channels -c conda-forge
  mamba env create --file "${CONDA_ENV_YAML}"
  conda install -y conda-pack --override-channels -c conda-forge
...
# Use model from SDXL_MODEL_SRC
export SDXL_MODEL_SRC="${HOME}/sdxl_cache/model_fp32"
export SDXL_MODEL_DIR="${LOCAL_SCRATCH_DIR}/sdxl_model_fp32"
cp -r "${SDXL_MODEL_SRC}" "${SDXL_MODEL_DIR}"
mlc run script --tags="get,ml-model,sdxl,_pytorch,_fp32,_rclone" --checkpoint="${SDXL_MODEL_DIR}"
